...
int main()
{
    ...
    //если предпоследняя цифра - чётная, то сдвиг вправо, если нечётная, то влево if (mode == 1)//Режим шифрования
    if (mode) {
        cout << "==========================================" << endl; cout << "ENCRYPTION MODE" << endl <<
        "==========================================" << endl;
        cout << "Please, input the initKey(cipher): "; cin.ignore();
        cin >> initKey;
        ...
        for (unsigned int i = 0; i < plainText.length();i += 2) {
            ...
            cipher = std::rand() % 32767; //генерация шифр-ключа cout << "cipher = " << cipher << endl; textInt = cipher ^ textInt; //поразрядный XOR
        ...
        cin >> initKey;
        srand(initKey);//генерация от суммы двух символов ключа cout << "Please, input the text you want to decrypt: "; cin >> cipherText;
        ...
        for (unsigned int i = 0; i < cipherText.length();i += 2) {
            ...
            textInt += tmpInt;
            cipher = std::rand() % 32767; //генерация шифр-ключа cout << "cipher = " << cipher << endl;
